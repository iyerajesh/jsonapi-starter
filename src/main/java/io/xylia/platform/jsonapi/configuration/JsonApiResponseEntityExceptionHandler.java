...
import io.xylia.platform.jsonapi.configuration.converters.ConstraintViolationConverter;
import io.xylia.platform.jsonapi.configuration.converters.ErrorConverter;
...
public class JsonApiResponseEntityExceptionHandler {
    ...
    protected ResponseEntity<Object> handleUnprocessableEntity(ResourceNotFoundException ex, WebRequest request) {
    ...
    protected ResponseEntity<Object> handleMethodArgumentNotValid(MethodArgumentNotValidException ex, WebRequest request) throws MethodArgumentNotValidException {
        if (canAcceptJsonApi(request)) {
            HttpStatus status = HttpStatus.UNPROCESSABLE_ENTITY;
            BindingResult bindingResult = ex.getBindingResult();
            List<Error> errors = bindingResult.getAllErrors().stream().filter(error -> error instanceof FieldError).map(FieldError.class::cast).map(this::newError)
     ...
     * <p>
     * The default implementation sets the {@link WebUtils#ERROR_EXCEPTION_ATTRIBUTE} request attribute
     * and creates a {@link ResponseEntity} from the given body, headers, and status.
     *
